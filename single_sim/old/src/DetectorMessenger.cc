...
{   
  ...
  guide_x1_cmd = new G4UIcmdWithADoubleAndUnit("/detector/guide_x1",this);
  guide_x1_cmd->SetGuidance("Set guide_x1 (positive)");
  guide_x1_cmd->SetParameterName("guide_x1",false);
  guide_x1_cmd->SetRange("guide_x1>=0. && guide_x1<1000.");
  guide_x1_cmd->SetDefaultUnit( "mm" );
  guide_x1_cmd->SetUnitCategory("Length");
  guide_x1_cmd->AvailableForStates(G4State_PreInit,G4State_Idle);

  guide_y1_cmd = new G4UIcmdWithADoubleAndUnit("/detector/guide_y1",this);
  guide_y1_cmd->SetGuidance("Set guide_y1 (positive)");
  guide_y1_cmd->SetParameterName("guide_y1",false);
  guide_y1_cmd->SetRange("guide_y1>=0. && guide_y1<1000.");
  guide_y1_cmd->SetDefaultUnit( "mm" );
  guide_y1_cmd->SetUnitCategory("Length");
  guide_y1_cmd->AvailableForStates(G4State_PreInit,G4State_Idle);

  guide_x2_cmd = new G4UIcmdWithADoubleAndUnit("/detector/guide_x2",this);
  guide_x2_cmd->SetGuidance("Set guide_x2 (positive)");
  guide_x2_cmd->SetParameterName("guide_x2",false);
  guide_x2_cmd->SetRange("guide_x2>=0. && guide_x2<1000.");
  guide_x2_cmd->SetDefaultUnit( "mm" );
  guide_x2_cmd->SetUnitCategory("Length");
  guide_x2_cmd->AvailableForStates(G4State_PreInit,G4State_Idle);

  guide_y2_cmd = new G4UIcmdWithADoubleAndUnit("/detector/guide_y2",this);
  guide_y2_cmd->SetGuidance("Set guide_y2 (positive)");
  guide_y2_cmd->SetParameterName("guide_y2",false);
  guide_y2_cmd->SetRange("guide_y2>=0. && guide_y2<1000.");
  guide_y2_cmd->SetDefaultUnit( "mm" );
  guide_y2_cmd->SetUnitCategory("Length");
  guide_y2_cmd->AvailableForStates(G4State_PreInit,G4State_Idle);
...
{ 
  ...
  if( command == guide_x1_cmd )
    detector->set_guide_x1( guide_x1_cmd->GetNewDoubleValue(newValue));

  if( command == guide_y1_cmd )
    detector->set_guide_y1( guide_y1_cmd->GetNewDoubleValue(newValue));

  if( command == guide_x2_cmd )
    detector->set_guide_x2( guide_x2_cmd->GetNewDoubleValue(newValue));

  if( command == guide_y2_cmd )
    detector->set_guide_y2( guide_y2_cmd->GetNewDoubleValue(newValue));
