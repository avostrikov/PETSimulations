...
void Detector::Materials()
{
  ...
  //implement LSO emission spectra

  const int numentry2 = 17;
  G4double lso_pp2[numentry2]            = { 2.296*eV, 2.340*eV, 2.385*eV, 2.431*eV, 2.480*eV,
                        2.531*eV, 2.583*eV, 2.638*eV, 2.696*eV, 2.756*eV,
                        2.818*eV, 2.884*eV, 2.952*eV, 3.024*eV, 3.100*eV,
                        3.179*eV, 3.263*eV};

  G4double lso_rindex[numentry2]        = { 1.82, 1.82, 1.82, 1.82, 1.82,
                        1.82, 1.82, 1.82, 1.82, 1.82,
                        1.82, 1.82, 1.82, 1.82, 1.82,
                        1.82, 1.82};

  G4double lso_abslength[numentry2]     = { 50.*m, 50.*m, 50.*m, 50.*m, 50.*m,
                        50.*m, 50.*m, 50.*m, 50.*m, 50.*m,
                        50.*m, 50.*m, 50.*m, 50.*m, 50.*m,
                        50.*m, 50.*m};

  G4double lso_fastcomponent[numentry2] = { 0.02, 0.04, 0.06, 0.08, 0.10, 0.15, 0.20, 0.30, 0.40, 0.50,
                        0.60, 0.70, 0.80, 0.65, 0.50, 0.25, 0.10};
  ...
  // following doesn't take into account LSO emission spectra:
